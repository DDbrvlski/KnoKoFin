    ...
    public class InvoiceDocument : IDocument
    {
        ...
        void ComposeTable(IContainer container)
        {
            ...
            container.Table(table =>
            {
                ...
                foreach (var item in Model.InvoiceProductsData)
                {
        ...
        void ComposeContent(IContainer container)
        {
            container.PaddingVertical(40).Column(column =>
            {
                ...
                column.Item().Row(row =>
                {
                    row.RelativeItem().Component(new SellerAddressComponent("Sprzedawca", Model.SellerData));
                    row.ConstantItem(50);
                    row.RelativeItem().Component(new BuyerAddressComponent("Nabywca", Model.CustomerData));
                });
                ...
                column.Item().Row(row =>
                {
                    row.RelativeItem().Component(new PaymentDetailsComponent(Model.AdditionalInfoData.PaymentMethodName, Model.AdditionalInfoData.PaymentDate, Model.AdditionalInfoData.DeliveryName, Model.AdditionalInfoData.CurrencyName, Model.AdditionalInfoData.DeliveryPrice));
                    row.ConstantItem(80);
                    row.RelativeItem().Component(new PaymentComponent(Model.InvoiceProductsData, Model.AdditionalInfoData.DeliveryPrice));
                });
